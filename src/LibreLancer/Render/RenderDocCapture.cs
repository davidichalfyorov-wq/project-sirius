...
public static class RenderDocCapture
{
    ...
    public static void Initialize()
    {
        ...
        // (entries 19/21) with NULL device+window = "capture anything".
        ...
        if (available)
        {
            FLLog.Info("RenderDoc", "In-app capture API attached (F12 / SIRIUS_CAPTURE_FRAME)");
    ...
    public static unsafe void Tick()
    {
        ...
        {
            capturing = false;
            var ok = ((delegate* unmanaged[Cdecl]<IntPtr, IntPtr, uint>)endCapture)(IntPtr.Zero, IntPtr.Zero);
        ...
        if (pending || frameCounter == captureFrame)
        {
            ...
            capturing = true;
            FLLog.Info("RenderDoc", $"Capture started (frame {frameCounter})");
        }
    }
}
